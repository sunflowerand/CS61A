...
def is_swap(player_score, opponent_score):
    ...
    plyaer_unit = player_score % 10
    opponent_unit = opponent_score % 10
    opponent_dec = opponent_score // 10 % 10

    subs = abs(plyaer_unit - opponent_unit)
    if(subs == opponent_dec):
        return True
    else:
        return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    dice:       A function 1of zero arguments that simulates a dice roll.
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    #使用高阶函数的技巧可以保留上一次的参数
    def sub_func(*score_list):
        gain = score_list[who] - last_score
        if gain > running_high:
            print(f"{gain} point(s)! That's the biggest gain yet for Player {who}")
            return announce_highest(who, score_list[who], gain)
        else:
