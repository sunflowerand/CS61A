...
def about(subject):
    ...
    def sub_func(text):
        ...
        textlist = [lower(i) for i in text_list]
        for i in subject:
            compare_a = lower(i)
            for j in textlist:
                if compare_a == j:
...
def accuracy(typed, source):
    ...
    if typed_words == [] and source_words == []:
        return 100.0
    elif (typed_words == [] and source_words != []) or (typed_words != [] and source_words == []):
        return 0.0
    total = 0.0
    for i in range(0, min(len(typed_words), len(source_words))):
        if typed_words[i] == source_words[i]:
            total += 1.0
    return (total / float(len(typed_words))) * 100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        for i in range(len(word_list)):
            diff_limit += [diff_function(typed_word, word_list[i], limit)]
        # n = limit + 1
        # for j in range(len(diff_limit)):
        #     n = min(diff_limit[j], n)
        n = 0
        n = min(diff_limit)
        if n > limit:
            return typed_word
